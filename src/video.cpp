...
int stream_component_open(VideoState *vidState, int stream_index) {
	...
	switch(codecCtx->codec_type) {
		...
		case AVMEDIA_TYPE_VIDEO:
			...
			vidState->video_tid = SDL_CreateThread(video_thread, "video_thread", vidState);
...
//	screen = SDL_SetVideoMode(640, 480, 0, 0);
	strncpy_s(vidState->filename, filepath, sizeof(vidState->filename));
	...
	vidState->parse_tid = SDL_CreateThread(decode_thread, "decode_thread", vidState);
...
int videoGetWidth()
{
	return global_video_state->pictq[0].width;
}


int videoGetHeight()
{
	return global_video_state->pictq[0].height;
}
